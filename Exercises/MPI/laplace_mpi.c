...
int main(int argc, char *argv[]) {
    ...
    while ( dt_global > MAX_TEMP_ERROR && iteration <= max_iterations ) {
        ...
        if(my_PE_num == 0)
        {
          MPI_Ssend(&Temperature[250][1], COLUMNS-2,MPI_DOUBLE,
            my_PE_num+1, DOWN, MPI_COMM_WORLD);
        }
        else
        {
          MPI_Recv(&Temperature_last[0][1],COLUMNS-2,MPI_DOUBLE,
        ...
        if(my_PE_num == npes-1)
        {
          MPI_Ssend(&Temperature[1][1], COLUMNS-2, MPI_DOUBLE,
           my_PE_num-1, UP, MPI_COMM_WORLD);
        }
        else
        {
          MPI_Recv(&Temperature_last[251][1], COLUMNS-2, MPI_DOUBLE,
...
void initialize(int npes, int my_PE_num){
    ...
    for(i = 0; i <= ROWS+1;i++)
    {
      ...
      Temperature_last[i][COLUMNS+1] = 
        ((ROWS_GLOBAL/(double)npes*my_PE_num)+i)*(100.0/ROWS_GLOBAL);
//      ((double)i+(ROWS_GLOBAL/(double)npes*my_PE_num))/(double)(COLUMNS+1);
    }
}
